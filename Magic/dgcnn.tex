\section{Algorithm Description}\label{sec:DGCNN}
...
\subsubsection{Graph Convolution Layer(s)}

In the first stage, a \textit{graph convolution} technique propagates each vertex's attributes to its neighborhood based on the structural connectivity.
To aggregate multi-scale substructural attributes, multiple graph
...
\subsubsection{SortPooling Layer}
Intuitively $\mathbf{Z}^{1:h}$ has $n$ rows and $\sum_{1}^{h}c_t$ column, which
...
\subsubsection{Remaining Layer}
In the last stage, the authors of the original DGCNN\cite{Dgcnn} append a one-dimension convolution (Conv1D) layer of kernel size $\sum_{1}^{h}c_t$ and stride size $\sum_{1}^{h}c_t$.
...
Equivalently, Equation~(\ref{equ:WeightedVertices}) computes $E$, the embedding of the graph obtained through a weighted summation of vertex embeddings \cite{GraphEmbedding}.
...
% \begin{figure}[htbp]
% \centerline{\includegraphics[width=0.48\textwidth]{Magic/figures/WeightedVertice.eps}}
% \caption{Weight vertices layer aggregates vertex embeddings, e.g. the output of sort pooling layer $\mathbf{Z}^{sp}$, into graph embedding $E$. $V_1, V2, V3, ...,$ and $ V_k$ in figure are feature descriptors of sort pooled vertices, represented as rows in $\mathbf{Z}^{sp}$.}
% \label{fig:WeightedVertice}
% \end{figure}
...
% \begin{figure}[htbp]
% \centerline{\includegraphics[width=0.48\textwidth]{Magic/figures/AdaptiveMaxPooling.eps}}
% \caption{An example of $3\times 3$ adaptive max pooling layer with
% two two-dimension inputs with different sizes.
% For the first input of size $4\times 4$, adaptive max pooling's kernel size = $2 \times 2$(shown as red shadow), padding = 0, stride = $1 \times 1$.
% For the second input of size $5\times 7$, adaptive max pooling's kernel size = $3 \times 3$(shown as red shadow), padding = 0, stride = $2 \times 1$.}
% \label{fig:AdaptiveMaxPooling}
% \end{figure}
